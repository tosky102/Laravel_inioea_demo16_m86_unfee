<template>
  <div style="outline: none; margin-left: auto; margin-right: auto; margin-bottom: 20px">
    <div class="image-container position-relative text-center" v-if="!images.length">
      ...
      <div v-else class="image-center position-absolute display-flex flex-column justify-content-center align-items-center" @dragover.prevent="onDragover">
        ...
        <div class="image-input position-absolute full-width full-height">
          <label for="image-upload-drawing" class="full-width full-height cursor-pointer">
          </label>
        </div>
      </div>
    </div>
    ...
    <div class="image-container position-relative text-center image-list" v-else>
      <div class="preview-image full-width position-relative cursor-pointer" @click="openGallery(currentIndexImage)" @drop="onDrop">
...
<!--          <input type="checkbox" v-model="images[currentIndexImage].drawing" true-value=1 false-value=0/><label class="label-drawing">図面</label>-->
    ...
    <div class="image-list-container display-flex flex-wrap" v-if="images.length && multiple">
      ...
      <div class="image-list-item position-relative cursor-pointer display-flex justify-content-center align-items-center" v-if="images.length < maxImage">
        ...
        <div class="input-add-image position-absolute full-width full-height">
          <label for="image-upload-drawing" class="display-block full-width full-height cursor-pointer">
          </label>
        </div>
      </div>
    </div>
    <div>
      <input id="image-upload-drawing" @change="uploadFieldChange" :multiple="multiple" :accept="accept" type="file">
      <input id="image-edit" @change="editFieldChange" :accept="accept" type="file">
    </div>
...
export default {
  ...
  props: {
    ...
    popupText: {
      type: String,
      default: 'この画像はデフォルトとして表示されます'
    },
    dropText: {
      type: String,
      default: 'ここにファイルをドロップしてください ...'
    },
    accept: {
      type: String,
  ...
  methods: {
    onDrop (e) {
      ...
      if (document.getElementById('image-upload-drawing')) {
        document.getElementById('image-upload-drawing').value = []
      }
    },
    onDragover () {
    ...
    createImage (file) {
      ...
      if (file.size > this.maxSize * 1024 * 1024) {
        alert(this.maxSize + 'MB以上の画像はアップロードできません。'); return;
      } else {
      ...
      reader.onload = (e) => {
        ...
        if (dataURI) {
          if (!this.images.length) {
            this.images.push({name: file.name, path: dataURI, highlight: 1, default: 1, drawing: 1})
            this.currentIndexImage = 0
          } else {
            this.images.push({name: file.name, path: dataURI, highlight: 0, default: 0, drawing: 1})
          }
