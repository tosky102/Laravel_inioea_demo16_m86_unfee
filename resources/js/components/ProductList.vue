  ...
  <div class="list-page-wrapper" ref="list_page_wrapper">
    ...
    <div class="list-page-content card login-card">
      <div class="list-page-search px-0 py-0 mb-0">
        <form action="/item" class="form" method="get">
          <input type="hidden" name="type" :value="config.type" />
          <div class="form-wrapper row">
            <div class="col-6 col-md-4 mb-2">
              <label for="genre" class="d-flex align-items-center">
                <select
                  ...
                  id="genre"
                >
                  <option value="">お仕事ジャンルを選択してください</option>
                  <option
                    v-for="(genre, index) in config.genres"
                    :key="genre"
                    :value="genre"
                  >
                    {{ genre }}
            ...
            <div class="col-6 col-md-4 mb-2">
              <label for="" class="d-flex align-items-center">
                <select
                  v-model="config.offer"
                  name="offer"
                  class="form-control login-input"
                  id="offer"
                >
                  <option value="">同伴者への提供を選択してください</option>
                  <option
                    v-for="(offer, key) in config.offers"
                    :key="key"
                    :value="key"
                  >
                    {{ offer }}
            ...
            <div class="col-6 col-md-4 mb-2">
              <label for="sns" class="d-flex align-items-center">
                <select
                  v-model="config.sns"
                  name="sns"
                  class="form-control login-input"
                  id="sns"
                >
                  <option value="">応募条件SNS名を選択してください</option>
                  <option
                    v-for="(sns, key) in config.postSNS"
                    :key="key"
                    :value="key"
                  >
                    {{ sns }}
            ...
            <div class="col-6 col-md-2 mb-2 d-flex align-items-center justify-content-start pl-8">
              <label for="is_emergency" class="form-check-label d-flex align-items-center justify-content-start ml-8 mb-2" style="margin-left: 1.25rem !important;">
                <input type="checkbox" v-model="config.isEmergency" name="is_emergency" id="is_emergency" class="form-check-input" :value="config.isEmergency" />
                <span>急募</span>
              </label>
            </div>
          </div>
