  ...
  <div id="detail-wrapper">
    <div id="detail-container">
      <div class="container-lg" style="margin: auto">
        ...
        <h2 class="detail-page-title">インフルエンサー情報</h2>

        <div class="row detail-page-container" style="margin: 1px" v-if="u_item">
          <div class="card login-card col-md-12" style="margin: 0">
            <div style="text-align: center; margin: 50px 5% 30px">
              <div class="detail-col-level-container" v-if="u_item.is_picked || u_item.level !== null && u_item.level.length > 0">
                <div v-if="u_item.is_picked" class="detail-col-level-item badge-picked">運営認定</div>
                <div class="detail-col-level-item" v-for="level in u_item.level" :key="level" v-if="u_item && u_item.level" :class="getLevelClass(level)">{{ level }}</div>
              </div>
              <h2>{{ u_item.name }}</h2>
            </div>
            ...
            <div class="detail-col-image">
              <div
                ...
                <img
                  :src="u_item.mainImage"
                  @click="openShowLightBox"/>
              </div>
              <div class="detail-sub-images-container">
                <div class="detail-sub-image" v-for="subImage in u_item.subImages" :style="{
                    width: detailSubImageWidth + 'px',
                    height: detailSubImageHeight + 'px',
                  }"
                  @click="selectSubImage(subImage)"
                  style="min-width: 100px; min-height: 100px" v-if="u_item && u_item.subImages">
            ...
            <div class="detail-action-container">
              <a
                v-if="u_item.role === 'company' && u_item.user_id !== u_item.sellerId && u_item.newMessageUrl"
            ...
            <div class="detail-col-sns" style="margin-bottom: 3%">
              <div class="detail-sub-sns-container">
                <div v-for="sns in u_item.sns">
                  <a :href="sns.url" v-if="u_item && u_item.sns">
                    <img v-if="sns.name !== '他'" :src="sns.icon"/>
                    <span v-else>その他</span>
                  </a>
                    <p>{{ sns.account }}</p>
                    <p>フォロワー数：{{ sns.followers }}人</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="detail-content" style="padding-top: 10px">
                <div class="detail-content-sub-container">
                  ...
                  <div
                    ...
                    style="margin: 1rem 0; padding: 50px 10%; font-size:1rem; display: block;">
                    <div class="detail-item" v-if="u_item.name">
                      <div class="detail-item-label">ニックネーム</div>
                      <div class="detail-item-value">
                        {{ u_item.name }}
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.gender">
                    ...
                    <div class="detail-item" v-if="u_item.main_category">
                      <div class="detail-item-label">カテゴリー</div>
                      <div class="detail-item-value">
                        {{ u_item.main_category }}
                      </div>
                    </div>

                    <div class="detail-item"v-if="u_item.birthplace">
                      <div class="detail-item-label">出身地</div>
                      <div class="detail-item-value">
                        {{ u_item.birthplace }}
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.residence">
                      <div class="detail-item-label">居住地</div>
                      <div class="detail-item-value">
                        {{ u_item.residence }}
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.area">
                      <div class="detail-item-label">活動エリア</div>
                      <div class="detail-item-value">
                        {{ u_item.area }}
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.specialty">
                      <div class="detail-item-label">特技</div>
                      <div class="detail-item-value" style="overflow-wrap: anywhere" v-html="u_item.specialty">
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.hobby">
                      <div class="detail-item-label">趣味</div>
                      <div class="detail-item-value" style="overflow-wrap: anywhere" v-html="u_item.hobby">
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.qualification">
                      <div class="detail-item-label">資格 / 免許</div>
                      <div class="detail-item-value" style="overflow-wrap: anywhere" v-html="u_item.qualification">
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.language">
                      <div class="detail-item-label">話せる言語</div>
                      <div class="detail-item-value">
                        {{ u_item.language }}
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.comment">
                      <div class="detail-item-label">自己PR</div>
                      <div class="detail-item-value" style="overflow-wrap: anywhere" v-html="u_item.comment">
                      </div>
                    </div>
                  </div>

                  <h2 class="detail-page-title" v-if="u_item.career_url_1 && u_item.career_1 || u_item.career_url_2 && u_item.career_2 || u_item.career_url_3 && u_item.career_3">活動実績</h2>
                  <div
                    class="row card login-card"
                    style="margin: 1rem 0; padding: 50px 10%; font-size:1rem; display: block;"
                    v-if="u_item.career_url_1 && u_item.career_1 || u_item.career_url_2 && u_item.career_2 || u_item.career_url_3 && u_item.career_3">
                    ...
                    <div class="detail-item" v-if="u_item.career_1">
                      <div class="detail-item-label" v-if="u_item.career_1">実績</div>
                      <div class="detail-item-value" v-if="u_item.career_1" v-html="u_item.career_1">
                      </div>
                    </div>
                    ...
                    <div class="detail-item" v-if="u_item.career_2">
                      <div class="detail-item-label" v-if="u_item.career_2">実績</div>
                      <div class="detail-item-value" v-if="u_item.career_2" v-html="u_item.career_2">
                      </div>
                    </div>
                    ...
                    <div class="detail-item" v-if="u_item.career_3">
                      <div class="detail-item-label" v-if="u_item.career_3">実績</div>
                      <div class="detail-item-value" v-if="u_item.career_3" v-html="u_item.career_2">
                      </div>
                    </div>
                  </div>

                  <h2 class="detail-page-title">運営担当者評価</h2>
                  <div
                    class="row card login-card"
                    style="margin: 1rem 0; padding: 50px 10%; font-size:1rem; display: block;">
                    <div class="detail-item" v-if="u_item.admin_comment">
                      <div class="detail-item-label">運営担当者コメント</div>
                      <div class="detail-item-value" style="overflow-wrap: anywhere" v-html="u_item.admin_comment">
                      </div>
                    </div>

                    <div class="detail-item" v-if="u_item.admin_pickup_category">
                      <div class="detail-item-label">得意ジャンル</div>
                      <div class="detail-item-value">
                        {{ u_item.admin_pickup_category }}
                      </div>
                    </div>

                    <div class="detail-item">
                      <div class="detail-item-label">運営認定</div>
                      <div class="detail-item-value">
                        {{ u_item.is_picked ? '運営認定' : '運営認定なし' }}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style="margin-bottom: 3%">
                  <div class="detail-action-container">
                    <a
                      v-if="u_item.role === 'company' && u_item.user_id !== u_item.id && u_item.newMessageUrl"
                      class="btn btn-call btn-pill w-100 btn-danger mx-2"
                      @click="sendNewMessage()"
                      style="background: linear-gradient(90deg, #ef0505, #e38300)"
                      >メッセージを送る</a>
...
export default {
  ...
  computed: {
    youtubeVideoId() {
      if (this.u_item.youtube) {
        const urlParts = this.u_item.youtube.split('v=');
  ...
  data() {
    return {
      ...
      u_item: JSON.parse(this.user),
      r_users: JSON.parse(this.relationusers),
  ...
  methods: {
    ...
    selectSubImage(subImage) {
      if (subImage) {
        this.u_item.mainImage = subImage;
      }
    },
    ...
    handleImages() {
      // this.images.push({path: this.u_item.mainImage});
      if (this.u_item && this.u_item.subImages) {
        for (var i in this.u_item.subImages) {
          var subImage = this.u_item.subImages[i];
          this.images.push({
            path: subImage,
          });
        }
    ...
    cartItem() {
      if (this.u_item.user_id == this.u_item.sellerId) {
        alert("自分の求人は購入できません");
      } else {
        if (this.u_item.status == 0) {
          alert("一時停止された求人です。販売者にお問い合わせください");
        } else {
          location.href = this.u_item.cartUrl;
        }
      }
    },
    twitterOpen() {
      window.open(this.u_item.twitterUrl, this.u_item.twitterSetting, 0);
    },
    sendNewMessage() {
      if (this.u_item.user_id == this.u_item.id) {
        alert("自分にはメッセージを送信できません。");
      } else {
        if (this.u_item.newMessageUrl) {
          location.href = this.u_item.newMessageUrl;
        }
      }
    },
    concernMessage() {
      if (this.u_item.user_id == this.u_item.sellerId) {
        ...
        location.href = this.u_item.concernUrl;
      }
    },
    review() {
      if (this.u_item.user_id == 0) {
...
.detail-col-level-container {
  ...
  .detail-col-level-item {
    padding: 10px 15px;
    color: #FFFFFF;
    border-radius: 18px;
  }

  .detail-col-level-item.badge-picked {
    background-color: #E1AE20;
  }

  // .detail-col-level-item.badge-bronze {
  //   background-color: #B87333;
  // }

  .detail-col-level-item.badge-silver {
    background-color: #A8AAAD;
  }

  .detail-col-level-item.badge-gold {
    background-color: #E1AE20;
  }

  .detail-col-level-item.badge-platinum {
    background-color: #B6528F;
  }

  .detail-col-level-item.badge-diamond {
